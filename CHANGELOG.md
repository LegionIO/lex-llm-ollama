...
### Changed
- **lex-llm 0.8.0 conformance** — legacy-to-canonical migration and the 0.8.0 contract cut across the gem:
  - **Legacy type rip** — `provider.rb` no longer constructs or consumes the deleted `Llm::Message`/`Chunk`/`Thinking`/`ToolCall`/`Content::Raw`/`Embedding` types. `parse_completion_response` is the one response-parse boundary returning `Canonical::Response` (08 R2); `build_chunk` is the one chunk-parse boundary returning `Canonical::Chunk`; the `to_legacy_*` re-canonicalization bridges are deleted. Streaming runs the shared `StreamAccumulator` (10 U1) and honors the one streaming contract: yields `Canonical::Chunk`, ends in exactly one `done` chunk (or an `error` chunk before the raise), returns the accumulated `Canonical::Response`; a non-200 NDJSON body now raises the typed error through the shared streaming failure path instead of being silently skipped into an empty "successful" stream.
  - **Renderer law (08 R1–R4)** — `render_payload` renders FROM canonical values with the 0.8.0 funnel signature (`params:` is `Canonical::Params | nil`, the `temperature:` kwarg is gone); the provider-spelled options keys (G18: `num_predict`, ...) are the translator's `options_for` edge mapping — the one params table in the gem. Tool arguments parse through the shared strict `Responses::ToolArguments.parse!` (10 U2): invalid JSON raises, the rescue-to-`{}` tolerance is deleted. Streaming tool-call deltas carry the accumulator fragment shape (`{ id:, name:, arguments: String, index: }`); Ollama's per-chunk complete-state re-send is handled by the accumulator's per-id replace semantics.
  - **Discovery read path (07 C5)** — the provider's legacy `discover_offerings` live-discovery override and its `Routing::ModelOffering` production (`offering_from_model` and helpers) are deleted; the base read path serves the activated inventory offerings from `Registry.snapshot`. The SSOT discovery actor remains the sole publication path (`OfferingDraft` + `WeightReconciler` under the writer mutex). The embedding operation returns the 05 §3 documented artifact (`{ text:, model:, embedding:, usage: }`) in place of the deleted `Llm::Embedding` object.
  - **Coordinator wiring rip** — the `Inventory::ScopedRefresher::LegacyCoordinatorAdapter` require and `Publisher` argument are removed from the discovery actor (the file is gone from lex-llm 0.8.0; the mixed-version projection window is over). The fleet worker runner passes `registry:` explicitly to `ProviderResponder.call` (fleet protocol v3: exact execution resolves the callable from the SSOT registry; the legacy `provider_class:`/`provider_instances:` arguments are gone).
  - **Callable contract** — `OllamaCallable#chat`/`#stream_chat` take `messages` positionally, matching the 0.8.0 base Provider funnel and the fleet worker dispatch shape; `count_tokens`/`embed` keep their keyword `messages:`/`text:`. The callable-side `enforce_canonical_messages!` calls (the one shared helper at the exact-execution boundary) are kept.
  - **Floor bump** — gemspec now requires `lex-llm >= 0.8.0`. `RULES.md` (the 0.8.0 architecture law) ships at the repo root, byte-for-byte.
  - **Specs** — the provider spec rewrites the streaming/parser coverage against canonical types; the SSOT v3 conformance suite runs the kit's B1 (central enforcement) and B2 (canonical outputs) groups against the real callable boundary; the dispatch-boundary regression block now drives rejection through the funnel/callable (the base enforces centrally — the per-provider render-seam check is deleted with the legacy Chat-facade entry it existed for).

### Fixed
- **Canonical dispatch boundary (2026-08-19 incident)** — provider input is now canonical-only at both dispatch seams: `OllamaCallable`'s fleet dispatch ops (`chat`, `stream_chat`, `count_tokens`) call the lex-llm 0.7.7 base `Provider#enforce_canonical_messages!` before delegating, and the provider's render seam (`render_payload` → `enforce_render_messages!`) accepts only `Canonical::Message` (pipeline dispatch) or provider-native `Legion::Extensions::Llm::Message` (Chat facade), raising a loud `ArgumentError` for anything else. The lenient provider-side re-canonicalization — the path that masked the 2026-08-19 hash bypass (25/25 failed openai dispatches) — is removed: zero Hash tolerance, no bridges, no adapters. The Ollama wire payload and client request formats are unchanged.
- **lex-llm 0.7.7 floor** — the gemspec now requires `lex-llm >= 0.7.7` for `Provider#enforce_canonical_messages!` and the first-class canonical `cache_control` member; the bare registry `gem 'lex-llm'` line is replaced by a test-group path dependency on the adjacent `../lex-llm` checkout so the boundary work resolves against 0.7.7 during development.
- **Dispatch-boundary specs** — the SSOT v3 conformance suite gains a dispatch-boundary regression block: canonical input (including the `cache_control` member) renders through the production render seam without leaking the transport-only key onto the wire, plain-Hash input raises at both the fleet callable and the render seam, and provider-native `Message` input continues to render through the seam; the `RecordingOllamaProvider` stand-in delegates `enforce_canonical_messages!` to the real base implementation.
